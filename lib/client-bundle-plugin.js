...
export default function (inputOptions, outputOptions, resolveFileUrl) {
  let cache;
  let placeholderToEntryPointMap;
  let exportCounter;
  let clientBundle;
  let clientOutput;
  ...
  return {
    name: 'client-bundle',
    buildStart() {
      placeholderToEntryPointMap = new Map();
      exportCounter = 0;
    },
    ...
    load(id) {
      ...
      placeholderToEntryPointMap.set(exportCounter, realId);
    ...
    async buildEnd(error) {
      const entryPoints = [...placeholderToEntryPointMap.values()];
    ...
    resolveImportMeta(property, { moduleId, format }) {
      ...
      const id = placeholderToEntryPointMap.get(num);
